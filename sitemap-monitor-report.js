...
function build(summary, extra = {}) {
  ...
  const complete = summary.total > 0 && summary.scanned >= summary.total;
  ...
  const banner = complete
    ...
    : `<p style="margin:0 0 16px;padding:10px 14px;background:#fef2f2;border-left:3px solid #dc2626;color:#991b1b">Incomplete sweep — ${n(summary.scanned)} of ${n(summary.total)} sitemaps were checked. The rest were not compared.</p>`;
  ...
  const text = [`Sitemap Monitor — nightly sweep (${day})`, '',
    complete ? 'Full sweep: every monitored sitemap was re-checked.' : `INCOMPLETE: ${n(summary.scanned)} of ${n(summary.total)} sitemaps checked.`, '',
